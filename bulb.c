...
PROCESS(timer_bulb, "Bulb Timer");
PROCESS(bulb, "Bulb");
AUTOSTART_PROCESSES(&bulb);
...
void input_callback(const void *data, uint16_t len, const linkaddr_t *src, const linkaddr_t *dest) {
    ...
    }  else if ((char)rcv_msg.type == '2' && (char)rcv_msg.node == '1' && rcv_msg.id == parent_id && rcv_msg.data == 0) {
        // Start the bulb timer process if the conditions are met
	printf("Bulb ID %u is Processing....\n",node_id);
        process_start(&timer_bulb, NULL);
    }
}
    ...
    while (1) {
        ...
        if (ev == PROCESS_EVENT_TIMER) {
            if (data == &discovery_broadcast_timer) {
                ...
                discovery_msg.node = '4'; // Device type for bulbs
                    ...
                    while (1) {
                        ...
                        if (ev == PROCESS_EVENT_TIMER) {
                            if (data == &heartbeat_timer) {
                                ...
                                heartbeat_msg.node = '4'; // Bulb type
...
/*---------------------------------------------------------------------------*/
PROCESS_THREAD(timer_bulb, ev, data)
{
  ...
  PROCESS_BEGIN();
	...
	printf(" BULB ON \n");
    	...
    	PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&periodic_timer));	// periodic timer to switch bulb mode
